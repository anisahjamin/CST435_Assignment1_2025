...
# Controller host (Docker service name)
CONTROLLER_HOST = os.getenv("CONTROLLER_HOST", "controller-server")
...
def connect_with_retry(target, retries=10, delay=2):
    ...
    for i in range(retries):
        try:
            ...
            return textprocess_pb2_grpc.TextServiceStub(channel)
        except grpc.FutureTimeoutError:
            print(f"⏳ Waiting for server {target}... ({i+1}/{retries})")
            time.sleep(delay)
...
def main():
    ...
    # Connect to controller server with retry
    stub = connect_with_retry(CONTROLLER)
