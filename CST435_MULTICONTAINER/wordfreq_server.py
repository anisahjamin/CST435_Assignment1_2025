...
class WordFreqServicer(textprocess_pb2_grpc.TextServiceServicer):
    def WordFrequency(self, request, context):
        try:
            ...
            print(f"✅ Processed WordFrequency request: {request.filename} ({len(words)} words)")
            return textprocess_pb2.WordFreqResponse(freq=freq)
        except Exception as e:
            print(f"❌ Error processing WordFrequency request: {e}")
            return textprocess_pb2.WordFreqResponse(freq={})

def serve():
    server = grpc.server(futures.ThreadPoolExecutor(max_workers=4))
    textprocess_pb2_grpc.add_TextServiceServicer_to_server(WordFreqServicer(), server)
    server.add_insecure_port('[::]:50051')
    print("✅ WordFreq server running on port 50051")
    server.start()
